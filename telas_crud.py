...
=======
    ...
    window = sg.Window('Nova DPS - Clientes', layout)
>>>>>>> 1e9e5fefd58715e60599fd17a473965eefb83cd1
        ...
        if event == 'Listar':
            try:
                with engine.connect() as conn:
                    dados = conn.execute(text("SELECT id_cliente, nome FROM cliente ORDER BY id_cliente")).fetchall()
                    texto = "\n".join([f"ID: {d[0]} | Nome: {d[1]}" for d in dados])
        ...
        if event == 'Atualizar':
            ...
            if not id_cliente:
                ...
                try:
                    ...
                    sg.popup("Cliente atualizado com sucesso no banco de dados!")
                except Exception as e:
...
=======
    ...
    window = sg.Window('Nova DPS - Pedidos', layout)
>>>>>>> 1e9e5fefd58715e60599fd17a473965eefb83cd1
...
<<<<<<< HEAD
        if event == 'Atualizar':
            ...
            modelo = values['modelo_equipamento']
            status = values['status_pedido']
            ...
            if not id_pedido:
                ...
                try:
                    ...
                    sg.popup("Pedido updated com sucesso!")
                except Exception as e: 
                    sg.popup_error(f"Erro: {e}")
...
=======
    ...
    window = sg.Window('Nova DPS - Serviços', layout)
>>>>>>> 1e9e5fefd58715e60599fd17a473965eefb83cd1
...
<<<<<<< HEAD
        if event == 'Atualizar':
            ...
            tipo_reparo = values['tipo_reparo']
...
=======
    ...
    window = sg.Window('Nova DPS - Orçamentos', layout)
>>>>>>> 1e9e5fefd58715e60599fd17a473965eefb83cd1
...
<<<<<<< HEAD
        if event == 'Atualizar':
            ...
            if not id_orcamento:
                ...
                try:
                    with engine.begin() as conn:
                        query = text("UPDATE orcamento SET valor_estimado = :est, valor_final = :fin WHERE id_orcamento = :id")
                        conn.execute(query, {"est": valor_estimado, "fin": valor_final, "id": id_orcamento})
...
=======
    ...
    window = sg.Window('Nova DPS - Peças', layout)
>>>>>>> 1e9e5fefd58715e60599fd17a473965eefb83cd1
...
=======
    ...
    window = sg.Window('Nova DPS - Pagamentos', layout)
>>>>>>> 1e9e5fefd58715e60599fd17a473965eefb83cd1
...
<<<<<<< HEAD
        if event == 'Atualizar':
            id_pagamento = values['id_pagamento']
            valor_pago = values['valor_pago']
            forma_pagamento = values['forma_pagamento']
    
            if not id_pagamento:
                sg.popup_error("Digite o ID do Pagamento para atualizar!")
            else:
                try:
                    with engine.begin() as conn:
                        query = text("UPDATE pagamento SET valor_pago = :val, forma_pagamento = :forma WHERE id_pagamento = :id")
                        conn.execute(query, {"val": valor_pago, "forma": forma_pagamento, "id": id_pagamento})
                    sg.popup("Pagamento atualizado com sucesso!")
                except Exception as e: 
                    sg.popup_error(f"Erro: {e}")
...
=======
    window = sg.Window('Nova DPS - Relatórios', layout, element_justification='c')
>>>>>>> 1e9e5fefd58715e60599fd17a473965eefb83cd1
    ...
    while True:
        ...
        if event == 'Fila de Serviços Pendentes (RF06)':
            try:
                with engine.connect() as conn:
                    ...
                    # Adicionado o d[3] (Status do Serviço) para o técnico saber em que fase está
        ...
        if event == 'Balanço Financeiro (Orçamentos vs Pagos)':
            try:
                with engine.connect() as conn:
                    # Agrupa os pagamentos e soma o total pago por orçamento
                    query = text("""
                    ...
                    """)
                    dados = conn.execute(query).fetchall()
                    # Mostra o valor final cobrado vs quanto o cliente efetivamente já pagou (útil para parcelamentos)
        ...
        if event == 'Rastreabilidade de Peças por Equipamento':
            try:
                with engine.connect() as conn:
                    # Navega por 4 tabelas para descobrir o nome do equipamento onde a peça foi instalada
                    query = text("""
